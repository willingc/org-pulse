  ...
  open('#{end_date}_#{org}.md', 'w') { |f|
    ...
    repos.sort_by(&:stars).reverse.each do |repo|
      ...
      if commits.length > 0 || merged_pull_requests.length > 0 || closed_issues.length > 0
        ...
        f.puts "-  [#{commits.length} #{'commit'.pluralize(commits.length)}](https://github.com/#{repo.full_name}/compare/master@%7B#{Date.parse(start_date).to_time.to_i}%7D...master@%7B#{Date.parse(end_date).to_time.to_i}%7D)" if commits.length > 0
        f.puts "-  [#{closed_issues.length} closed #{'issue'.pluralize(closed_issues.length)}](https://github.com/#{repo.full_name}/issues?utf8=%E2%9C%93&q=is%3Aissue%20closed%3A#{start_date}..#{end_date})" if closed_issues.length > 0
        if merged_pull_requests.any?
          f.puts "\n#### Merged pull requests\n"
          merged_pull_requests.each do |pr|
            f.puts "- [#{pr.title}](#{pr.html_url}) by [#{pr.user.login}](#{pr.user.html_url})"
          end
        end
      end
    end
